      ...
      <td><p>&nbsp;</p>
        <form enctype="multipart/form-data" id="formthemtintuc" name="formthemtintuc" method="post" action="" >
            ...
            <td colspan="2">SỬA TIN TỨC</td>
                        ...
                        <tr>
                              ...
                              <td><label for="idtheloai"></label>
              				  ...
              				  ?>
                                <option <?php if( $row_theloai["idtheloai"]==$rowchitiet["idtheloai"] ) echo "selected='selected'"; ?> value="<?php echo $row_theloai['idtheloai'] ?>"><?php echo $row_theloai["tentheloai"] ?></option>
                                <?php
              				  }
              				  ?>
          ...
          <tr>
                ...
                <input type="text" value="<?php echo $rowchitiet['tieude'] ?>" name="TieuDe" id="TieuDe" /></td>
              </tr>
              <tr>
                <td>Tóm tắt</td>
                <td><label for="TomTat"></label>
                <textarea name="TomTat"  id="TomTat" cols="45" rows="5"><?php echo $rowchitiet['tomtat'] ?></textarea></td>
              </tr>
          ...
          <tr>
            <td>&nbsp;</td>
            <td><input  type="button" name="btnThem" id="btnThem" value="Sửa" /></td>
          </tr>
      ...
      $("#btnThem").click(function(){
        ...
        if(tieude == ""){
          ...
          if(!regextieude.test(tieude)){
           ...
           if(hinhanh == ""){
             $.post("kiemtrasuatintuc.php",{idtin:"<?php echo $rowchitiet["idtin"]?>",tieude:$('#TieuDe').val(),tomtat:$('#TomTat').val(),idtheloai:$('#idtheloai').val()},function(data){
                if(data == "true"){
                   ...
                   $.ajax({
                       ...
                       success: function (res) {
                         if(res == false){
                           ...
                           $.post("kiemtrasuatintuc.php",{idtin:"<?php echo $rowchitiet["idtin"]?>",tieude:$('#TieuDe').val(),tomtat:$('#TomTat').val(),hinhanh:res,hinhcu:"<?php
                            echo $_GET["hinhanh"];?>",idtheloai:$('#idtheloai').val()},function(data){
                              if(data == "true"){
                                alert('Sửa thành công');
                                let url = 'http://192.168.64.2/baitaplon/baitap_175A071324_175A071192/Quantri/listtintuc.php';
                               $('#formthemtintuc').prop('action',url);
...
<?php
 ...
 <?php
  $idtin = $_GET["idtin"]; settype($idtin,"int");
  $rowchitiet = chitiettinbydanhmuc($idtin);
  ?>
 <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
 <html xmlns="http://www.w3.org/1999/xhtml">
   <head>
           ...
           ?>
                         ...
                         <tr>
                               ...
                               <td><label for="idtheloai"></label>
                                 <select name="idtheloai" id="idtheloai">
                                 <?php
               				  $theloai =DanhSachTheLoaiTheoDanhMuc($rowchitiet["iddanhmuc"]);
               				  while($row_theloai = mysqli_fetch_array($theloai,MYSQLI_ASSOC)){
